...
// `zh` catches Chinese preferences with no exact catalog (zh_SG, a bare zh), which Chrome would
// otherwise resolve to English. It is a copy of zh_CN written by `sync-zh`, never edited by hand.
const ZH = join(LOCALES_DIR, 'zh', 'messages.json');
const ZH_CN = join(LOCALES_DIR, 'zh_CN', 'messages.json');

function zhDiffers() {
  if (!existsSync(ZH_CN)) return false;
  return JSON.stringify(readJson(ZH, null)) !== JSON.stringify(readJson(ZH_CN, null));
}

function syncZh() {
  copyFileSync(ZH_CN, ZH);
  console.log('zh/messages.json: copied from zh_CN');
}
...
function check() {
  ...
  if (zhDiffers()) {
    failed = true;
    console.error('zh/messages.json differs from zh_CN; run `node scripts/i18n.mjs sync-zh`');
  }
...
else if (command === 'sync-zh') syncZh();
else if (command === 'review') review();
else {
  console.error('usage: node scripts/i18n.mjs <check|sync-zh|review>');
  process.exit(2);
}
