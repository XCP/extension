...
walletTest.describe('Connected Sites Page (/settings/connected-sites)', () => {
  ...
  walletTest('shows empty state when no sites connected', async ({ page }) => {
    ...
    const hasNoConnected = await page.locator('text=/No connected/i').first().isVisible({ timeout: 2000 }).catch(() => false);

    expect(hasEmptyState || hasEmptyMessage || hasNoConnected).toBe(true);
  });
