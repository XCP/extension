...
walletTest.describe('Show Passphrase Page (/secrets/show-passphrase)', () => {
  ...
  walletTest('page loads with wallet ID', async ({ page }) => {
    ...
    if (walletId) {
      ...
      const hasWarning = await page.locator('text=/Warning|Never share|recovery phrase/i').first().isVisible({ timeout: 5000 }).catch(() => false);
  ...
  walletTest('shows security warning', async ({ page }) => {
    ...
    if (walletId) {
      ...
      const hasWarning = await page.locator('text=/warning|never share|do not share|keep secret|backup/i').first().isVisible({ timeout: 5000 }).catch(() => false);
  ...
  walletTest('reveals passphrase with correct password', async ({ page }) => {
    ...
    if (walletId) {
      ...
      // Should show 12-word phrase (words separated by spaces)
      const hasWords = await page.locator('text=/\\b\\w+\\s+\\w+\\s+\\w+\\s+\\w+/').first().isVisible({ timeout: 5000 }).catch(() => false);
      const hasWordList = await page.locator('[data-testid*="word"], .word-item, .seed-word').first().isVisible({ timeout: 3000 }).catch(() => false);

      expect(hasWords || hasWordList).toBe(true);
    }
  });
  ...
  walletTest('shows error for wrong password', async ({ page }) => {
    ...
    if (walletId) {
      ...
      const noPhrase = !(await page.locator('[data-testid*="word"], .seed-word').first().isVisible({ timeout: 1000 }).catch(() => false));

      expect(hasError || noPhrase).toBe(true);
    }
  });
  ...
  walletTest('handles invalid wallet ID', async ({ page }) => {
    ...
    const redirected = !page.url().includes('/secrets/show-passphrase');
