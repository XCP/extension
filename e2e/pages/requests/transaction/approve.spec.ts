 ...
 *
 * Tests for /requests/transaction/approve route
 *
...
walletTest.describe('Approve Transaction Page (/requests/transaction/approve)', () => {
  walletTest('page loads without crashing', async ({ page }) => {
    await page.goto(page.url().replace(/\/index.*/, '/requests/transaction/approve'));
  ...
  walletTest('page shows appropriate state without pending transaction', async ({ page }) => {
    await page.goto(page.url().replace(/\/index.*/, '/requests/transaction/approve?origin=https://test.example.com&requestId=test-123'));
    await page.waitForLoadState('networkidle');
  ...
  walletTest('page is accessible (no 404)', async ({ page }) => {
    const response = await page.goto(page.url().replace(/\/index.*/, '/requests/transaction/approve'));
  ...
  walletTest('shows error or close when transaction not found', async ({ page }) => {
    await page.goto(page.url().replace(/\/index.*/, '/requests/transaction/approve?origin=https://test.example.com&requestId=invalid-123'));
    await page.waitForLoadState('networkidle');
  ...
  walletTest('shows domain when origin is provided', async ({ page }) => {
    ...
    await page.goto(page.url().replace(/\/index.*/, `/requests/transaction/approve?origin=${testOrigin}&requestId=test-123`));
    await page.waitForLoadState('networkidle');
