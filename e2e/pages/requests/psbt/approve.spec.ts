 ...
 *
 * Tests for /requests/psbt/approve route
 *
...
walletTest.describe('Approve PSBT Page (/requests/psbt/approve)', () => {
  walletTest('page loads without crashing', async ({ page }) => {
    await page.goto(page.url().replace(/\/index.*/, '/requests/psbt/approve'));
  ...
  walletTest('page shows appropriate state without pending PSBT', async ({ page }) => {
    await page.goto(page.url().replace(/\/index.*/, '/requests/psbt/approve?origin=https://test.example.com&requestId=test-123'));
    await page.waitForLoadState('networkidle');
  ...
  walletTest('page is accessible (no 404)', async ({ page }) => {
    const response = await page.goto(page.url().replace(/\/index.*/, '/requests/psbt/approve'));
