/**
 * Show Private Key Page Tests (/wallet/secrets/show-private-key/:walletId)
 *
...
walletTest.describe('Show Private Key Page (/secrets/show-private-key)', () => {
  ...
  walletTest('page loads with wallet ID', async ({ page }) => {
    ...
    await page.goto(page.url().replace(/\/index.*/, `/wallet/secrets/show-private-key/${walletId}`));
  ...
  walletTest('handles invalid wallet ID', async ({ page }) => {
    await page.goto(page.url().replace(/\/index.*/, '/wallet/secrets/show-private-key/invalid-wallet-id-12345'));
    await page.waitForLoadState('networkidle');
