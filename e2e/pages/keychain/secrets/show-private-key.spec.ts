/**
 * Show Private Key Page Tests (/keychain/secrets/show-private-key/:walletId)
 *
 * Tests for viewing the wallet's private key.
 * Requires password verification before revealing sensitive data.
 */

import { expect, getPrivateKeyRoute, TEST_PASSWORD, walletTest } from '@e2e/fixtures';
...
walletTest.describe('Show Private Key Page (/secrets/show-private-key)', () => {
  ...
  walletTest('back button navigates back', async ({ page }) => {
    ...
    await page.goto(page.url().replace(/\/keychain\/wallets.*/, `/keychain/secrets/show-private-key/${walletId}`));
    await page.waitForLoadState('networkidle');
