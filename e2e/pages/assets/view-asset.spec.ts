/**
 * View Asset Page Tests (/assets/view/:asset)
 *
...
walletTest.describe('View Asset Page (/assets/view/:asset)', () => {
  ...
  walletTest('displays asset name prominently', async ({ page }) => {
    ...
    if (page.url().includes('/assets/view')) {
  ...
  walletTest('handles invalid asset gracefully', async ({ page }) => {
    await page.goto(page.url().replace(/\/index.*/, '/assets/view/INVALID_ASSET_12345'));
    await page.waitForLoadState('networkidle');
    ...
    const redirected = !page.url().includes('/assets/view');
