/**
 * View Balance Page Tests (/assets/balance/:asset)
 *
...
walletTest.describe('View Balance Page (/assets/balance/:asset)', () => {
  ...
  walletTest('displays asset name', async ({ page }) => {
    ...
    if (page.url().includes('/assets/balance')) {
  ...
  walletTest('handles invalid asset gracefully', async ({ page }) => {
    await page.goto(page.url().replace(/\/index.*/, '/assets/balance/INVALID_ASSET_67890'));
    await page.waitForLoadState('networkidle');
    ...
    const redirected = !page.url().includes('/assets/balance');
