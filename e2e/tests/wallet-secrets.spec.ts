...
walletTest.describe('Show Passphrase Page (/show-passphrase)', () => {
  ...
  walletTest('can access show passphrase from wallet selection', async ({ page }) => {
    ...
    if (menuCount > 0 && await menuButton.isVisible()) {
      ...
      const optionCount = await showPassphraseOption.count();
  ...
  walletTest('show passphrase has copy button after reveal', async ({ page }) => {
    ...
    if (menuCount > 0 && await menuButton.isVisible()) {
      ...
      if (optionCount > 0) {
        ...
        // Should have copy button after revealing - could be text or icon
        const copyButton = page.locator('button:has-text("Copy")').or(page.locator('button[aria-label*="Copy"]')).or(page.locator('button svg')).first();
...
walletTest.describe('Show Private Key Page (/show-private-key)', () => {
  ...
  walletTest('can access show private key from address selection', async ({ page }) => {
    ...
    if (menuCount > 0 && await menuButton.isVisible()) {
      ...
      const optionCount = await showKeyOption.count();
