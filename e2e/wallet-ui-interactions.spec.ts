...
test.describe('Wallet UI Interactions', () => {
  test('refresh mnemonic button on create wallet', async () => {
    ...
    const context = await chromium.launchPersistentContext('test-results/refresh-mnemonic', {
      headless: false,
      args: [
  ...
  test('password show/hide on create wallet', async () => {
    ...
    const context = await chromium.launchPersistentContext('test-results/password-show-hide-create', {
      headless: false,
      args: [
        `--disable-extensions-except=${pathToExtension}`,
        `--load-extension=${pathToExtension}`,
  ...
  test('mnemonic show/hide on import wallet', async () => {
    ...
    const context = await chromium.launchPersistentContext('test-results/mnemonic-show-hide', {
      headless: false,
      args: [
        `--disable-extensions-except=${pathToExtension}`,
        `--load-extension=${pathToExtension}`,
    ...
    if (hasImportWallet) {
      ...
      await page.goto(`chrome-extension://${extensionId}/onboarding`);
      await page.waitForTimeout(1000);
      await page.getByText('Import Wallet').click();
    }
  ...
  test('password show/hide on import wallet', async () => {
    ...
    const context = await chromium.launchPersistentContext('test-results/password-show-hide-import', {
      headless: false,
      args: [
        `--disable-extensions-except=${pathToExtension}`,
        `--load-extension=${pathToExtension}`,
