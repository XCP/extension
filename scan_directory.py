...
def matches_filter(path, filter_keywords):
    """
    Checks if the path contains any of the filter keywords (case-insensitive).
    If filter_keywords is None or empty, returns True (no filtering).
    """
    if not filter_keywords:
        return True
    path_lower = path.lower()
    return any(keyword.lower() in path_lower for keyword in filter_keywords)
...
def generate_structure(root_dir, filter_keywords=None, skip_keywords=None):
    """
    Walk through root_dir and collect folder and file paths while skipping ignored items.
    Optionally filter by keywords in the path and skip paths with specified keywords.

    Args:
      - root_dir: The root directory to scan.
      - filter_keywords: Optional list of strings to filter paths (e.g., ["compose", "test"]).
    ...
    for dirpath, dirnames, filenames in os.walk(root_dir):
        ...
        if matches_filter(posix_dir, filter_keywords) and not should_skip(posix_dir, skip_keywords):
        ...
        for filename in filenames:
            ...
            if matches_filter(rel_file, filter_keywords) and not should_skip(rel_file, skip_keywords):
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate a project listing that first shows folder names and file paths, then lists file contents. "
                    ...
                    "'scan_directory.py' or 'package-lock.json'. Optionally filter by keywords in paths and skip paths with specified keywords."
    )
    parser.add_argument(
        "path",
    ...
    parser.add_argument(
        "--filter",
        help="Optional comma-separated keywords to filter folders and files (e.g., 'compose,test,src')."
    )
    parser.add_argument(
        "--skip",
    ...
    # Split the filter argument into a list if provided, otherwise None
    ...
    folders, files = generate_structure(root_dir, filter_keywords=filter_keywords, skip_keywords=skip_keywords)
