...
export function AssetSelectInput({
  ...
  useEffect(() => {
    const searchAssets = async () => {
      if (query.length < 2) {
        if (isInitialLoad && activeWallet?.pinnedAssetBalances) {
          const pinnedAssetsList = activeWallet.pinnedAssetBalances.map((asset) => ({
            asset,
            symbol: asset,
  ...
  }, [query, isInitialLoad, activeWallet?.pinnedAssetBalances]);

  const handleComboboxButtonClick = () => {
    if (query.length < 2 && activeWallet?.pinnedAssetBalances) {
      const pinnedAssetsList = activeWallet.pinnedAssetBalances.map((asset) => ({
        asset,
        symbol: asset,
        description: `${asset} Token`,
