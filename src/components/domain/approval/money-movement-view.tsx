 ...
 * stack, not here.
 */
export function MoneyMovementView({ movement, flexible }: MoneyMovementViewProps) {
  ...
  return (
    <>
      <div className="text-center mb-3">
        ...
        {incomplete && (
          <p className="text-xs text-warning-600 mt-1">Some amounts couldn't be determined — review the details.</p>
        )}
        {flexible && (
          <p className="text-xs text-gray-400 mt-1">More inputs or outputs may be added after you sign.</p>
        )}
      </div>
      <div className="pt-3 border-t border-gray-100 space-y-1.5 text-xs">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="font-medium text-gray-900 flex-shrink-0">{btc(fee)} BTC</span>
        </div>
      </div>
    </>
  );
