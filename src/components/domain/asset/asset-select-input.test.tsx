...
vi.mock("@/components/icons", () => ({
  FiChevronDown: ({ className }: any) => (
    <div data-testid="chevron-down" className={className} />
  ),
  FiCheck: ({ className }: any) => (
...
describe("AssetSelectInput", () => {
  ...
  it("should show help text when showHelpText is true", () => {
    ...
    expect(
      screen.getByText("Search and select an asset by name or symbol"),
    ).toBeInTheDocument();
  });
    ...
    await waitFor(
      () => {
        expect(global.fetch).toHaveBeenCalledWith(
          "https://api.xcp.io/v2/assets?query=XCP&limit=20",
        );
      },
      { timeout: 1000 },
    );
  ...
  it("should show asset icon for selected asset", () => {
    ...
    expect(icon).toHaveAttribute("src", "https://cdn.xcp.io/img/icon/BTC");
  });
