 ...
 *
 * The options are the shared address-type listbox: arrow keys move focus only, and Enter, Space
 * or a click commits. Escape and outside clicks close the popover through Headless UI.
 */
  ...
  return (
    <div className="space-y-2">
      ...
      <AddressFormatListbox
        ...
        busy={isLoadingPreviews}
        className="space-y-1"
        optionClassName={({ selected }) =>
          `flex w-full cursor-pointer items-center rounded px-2 py-1.5 outline-none hover:bg-gray-50 focus-visible:ring-2 focus-visible:ring-blue-500 ${selected ? 'bg-blue-50' : ''}`}
      >
        {(format, { selected }) => {
          ...
          return (
            <>
              ...
              <FaCheck
                className={`ml-2 size-4 shrink-0 text-blue-500 ${selected ? '' : 'opacity-0'}`}
                aria-hidden="true"
              />
            </>
          );
