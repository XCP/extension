 ...
 * Use this when you need the timeout duration for timers/alarms.
 */
 ...
 * Flat structure for ergonomic access: settings.fiat vs settings.preferences.fiat
 */
export interface AppSettings {
  // Schema version for migrations
  version?: number;

  // Session state (internal, not user-configured)
...
function normalizeSettings(settings: AppSettings): AppSettings {
  ...
  if (!PRICE_UNITS.includes(normalized.priceUnit)) {
    normalized.priceUnit = DEFAULT_SETTINGS.priceUnit;
  }
  if (!FIAT_CURRENCIES.includes(normalized.fiat)) {
    normalized.fiat = DEFAULT_SETTINGS.fiat;
  }
