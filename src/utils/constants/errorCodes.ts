...
export function getErrorMessage(code: number): string {
  const errorMessages: Record<number, string> = {
    // JSON-RPC errors
    [JSON_RPC_ERROR_CODES.PARSE_ERROR]: 'Parse error: Invalid JSON',
    [JSON_RPC_ERROR_CODES.INVALID_REQUEST]: 'Invalid request',
    [JSON_RPC_ERROR_CODES.METHOD_NOT_FOUND]: 'Method not found',
    [JSON_RPC_ERROR_CODES.INVALID_PARAMS]: 'Invalid parameters',
    [JSON_RPC_ERROR_CODES.INTERNAL_ERROR]: 'Internal error',
    [JSON_RPC_ERROR_CODES.SERVER_ERROR]: 'Server error',
    
    // Provider errors
    [PROVIDER_ERROR_CODES.USER_REJECTED]: 'User rejected the request',
    [PROVIDER_ERROR_CODES.UNAUTHORIZED]: 'Unauthorized',
    [PROVIDER_ERROR_CODES.UNSUPPORTED_METHOD]: 'Unsupported method',
    [PROVIDER_ERROR_CODES.DISCONNECTED]: 'Provider disconnected',
    [PROVIDER_ERROR_CODES.CHAIN_DISCONNECTED]: 'Chain disconnected',
    [PROVIDER_ERROR_CODES.CHAINS_NOT_ADDED]: 'Chains not added',
    
    // XCP Wallet errors
    [XCP_ERROR_CODES.WALLET_LOCKED]: 'Wallet is locked',
    [XCP_ERROR_CODES.NO_WALLET]: 'No wallet found',
    [XCP_ERROR_CODES.TRANSACTION_FAILED]: 'Transaction failed',
    [XCP_ERROR_CODES.INSUFFICIENT_BALANCE]: 'Insufficient balance',
    [XCP_ERROR_CODES.RATE_LIMITED]: 'Rate limit exceeded',
    [XCP_ERROR_CODES.INVALID_ADDRESS]: 'Invalid address',
    [XCP_ERROR_CODES.ASSET_NOT_FOUND]: 'Asset not found',
    [XCP_ERROR_CODES.NETWORK_ERROR]: 'Network error',
    [XCP_ERROR_CODES.NOT_CONNECTED]: 'Not connected to wallet',
    [XCP_ERROR_CODES.TIMEOUT]: 'Operation timed out',
  };
...
/**
 * Check if an error code indicates a user rejection
 */
export function isUserRejection(code: number): boolean {
  return code === PROVIDER_ERROR_CODES.USER_REJECTED;
}

/**
 * Check if an error code indicates a connection issue
 */
