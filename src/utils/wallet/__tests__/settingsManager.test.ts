...
const mockGetKeychainSettings = vi.mocked((await import('@/utils/storage')).getKeychainSettings);
const mockUpdateKeychainSettings = vi.mocked((await import('@/utils/storage')).updateKeychainSettings);

// Import the actual default settings
const { DEFAULT_KEYCHAIN_SETTINGS } = await import('@/utils/storage/settingsStorage');
