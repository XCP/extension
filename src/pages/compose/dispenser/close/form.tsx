          ...
          {relevantDispensers.length === 0 ? (
            <div className="relative w-full mt-1 cursor-not-allowed rounded-lg bg-gray-100 py-2.5 pl-3 pr-10 text-left border border-gray-300 text-gray-500 sm:text-sm">
              <span className="block truncate">
                {asset ? t('close_form_no_open_dispensers_found_for', [String(asset)]) : t('close_form_no_open_dispensers_found_for_2')}
              </span>
                  ...
                  <ListboxButton className="relative w-full cursor-pointer rounded-lg bg-gray-50 py-2.5 pl-3 pr-10 text-left border border-gray-200 outline-none focus:border-blue-500 focus-visible:ring-2 focus-visible:ring-blue-500 text-base disabled:bg-gray-100 disabled:cursor-not-allowed">
                    <div className="flex items-center">
                      {selectedDispenser?.asset && (
                        <AssetIcon asset={selectedDispenser.asset} />
                      )}
                      <span
                        className={`block truncate ${selectedDispenser ? "ml-2" : ""}`}
                      >
                        {selectedDispenser
                          ? selectedDispenser.asset
                  ...
                  <ListboxOptions className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                    {relevantDispensers.map((dispenser) => (
                      <ListboxOption
                        ...
                        className={({ focus }) =>
                          `relative cursor-pointer select-none py-2.5 pl-10 pr-4 ${focus ? "bg-blue-500 text-white" : "text-gray-900"}`
                        }
                      >
                        {({ selected, focus }) => (
                          <>
                ...
                <div className="mt-3 text-sm p-3 bg-gray-50 rounded-md border border-gray-200 space-y-2">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    >
                      ...
                      {selectedDispenser.tx_hash.slice(-6)}
                  ...
                  <input
                    type="hidden"
                    name="give_remaining_normalized"
                    value={selectedDispenser.give_remaining_normalized}
                  />
                </div>
              )}
              {showHelpText && (
