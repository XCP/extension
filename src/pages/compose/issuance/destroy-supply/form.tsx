...
export function DestroyForm({ onSubmit, initialFormData, initialAsset }: DestroyFormProps) {
  ...
  const { isLoading, error: assetError, data: assetDetails } = useAssetDetails(initialAsset);
  ...
  if (assetError || !assetDetails) return <div className="p-4 text-red-500">Error loading asset details: {assetError?.message}</div>;
